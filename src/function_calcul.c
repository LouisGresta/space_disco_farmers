...
// Format variable de retour => [collector1 ID, planet1 ID, collector2 ID, planet2 ID]
...
uint16_t *determineTargetPlanets(Ship collector1, Ship collector2, Planet *planets, uint8_t nb_planets)
{
    ...
    for (uint8_t i = 0; i < nb_planets; i++)
    {
        ...
        if (planets[i].saved == 0)
        {
            ...
            uint16_t dist2 = round(sqrt(pow(abs(collector2.x - planets[i].x), 2) + pow(abs(collector2.y - planets[i].y), 2)));
            ...
            // vérifier si la planète est la + proche de collector2 sans avoir same focus que collector1
            if (min_distance2 == -1 || (dist2 < min_distance2 && i != index_planet1))
            {
                min_distance2 = dist2;
                index_planet2 = i;
            }
