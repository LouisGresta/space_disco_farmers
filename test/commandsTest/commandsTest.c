...
MU_TEST(test_move_command) {
    mu_assert_string_eq("MOVE 1 90 1000\n", move_command(1, 90, 1000));
    mu_assert_string_eq("MOVE 6 180 2000\n", move_command(6, 180, 2000));
    mu_assert_string_eq("MOVE 8 270 3000\n", move_command(8, 270, 3000));
}

MU_TEST(test_fire_command) {
    mu_assert_string_eq("FIRE 1 90\n", fire_command(1, 90));
    mu_assert_string_eq("FIRE 3 180\n", fire_command(3, 180));
    mu_assert_string_eq("FIRE 5 270\n", fire_command(5, 270));
}

MU_TEST(test_radar_command) {
    mu_assert_string_eq("RADAR 6\n", radar_command(6));
    mu_assert_string_eq("RADAR 7\n", radar_command(7));
}
